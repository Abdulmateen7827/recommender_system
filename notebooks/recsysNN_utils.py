...
def load_data():
    ...
    with open('notebooks/data/ml-latest-small/content_item_train_header.txt', newline='') as f:
        item_features = list(csv.reader(f))[0]
    with open('notebooks/data/ml-latest-small/content_user_train_header.txt', newline='') as f:
...
#    with open('./data/movies.csv', newline='') as csvfile:
    with open('notebooks/data/ml-latest-small/content_movie_list.csv', newline='') as csvfile:
    ...
    return(item_train, user_train, y_train, item_features, user_features, movie_dict)
...
def pprint_train(x_train, features,  vs, u_s, maxcount=5, user=True):
    ...
    if vs < u_s:
        print(
            "error, vector start {vs} should be greater then user start {u_s}")
    for i in range(u_s):
        head[i] = "[" + head[i] + "]"
    genres = features[vs:]
    hdr = head + genres
...
def pprint_data(y_p, user_train, item_train, printfull=False):
    ...
    for i in range(0, 1000):
        ...
        print(f"{user_train[i,2].astype(float): 0.2f}, ",
              end='')       # rating ave
        print(": ", end='')
        print(f"{item_train[i,0].astype(int):d}, ",  end='')   # movie id
        print(f"{item_train[i,2].astype(float):0.1f}, ",
              end='')   # ave movie rating
        if printfull:
...
def print_pred_debug(y_p, y, user, item, maxcount=10, onlyrating=False,  printfull=False):
    ...
    for i in range(0, y_p.shape[0]):
        if onlyrating == False or (onlyrating == True and y[i, 0] != 0):
            ...
            print(f"{item[i, 2].astype(float):0.1f}, ",
                  end='')   # ave movie rating
            print(": ", end='')
            if printfull:
                for j in range(uvs, user.shape[1]):
                    print(f"{user[i, j].astype(float):0.1f}, ",
                          end='')  # rating
                print(":", end='')
...
def print_existing_user(y_p, y, user, items, item_features, ivs, uvs, movie_dict, maxcount=10):
    ...
    count = 0
    movies_listed = defaultdict(int)
    disp = [["y_p", "y", "user", "user genre ave",
             "movie rating ave", "title", "genres"]]
    listed = []
    count = 0
    ...
    table = tabulate.tabulate(disp, tablefmt='html', headers="firstrow", floatfmt=[
                              ".1f", ".1f", ".0f", ".2f", ".2f"])
    return(table)
